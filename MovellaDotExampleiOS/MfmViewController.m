 ...
 * Start MFM and Stop MFM is very easy, we can call startMFM and stopMFM directyly, then we need to listen the DotMFMDelegate interfaces.
 */
@interface MfmViewController ()<UITableViewDelegate, UITableViewDataSource, DotMFMDelegate, DotConnectionDelegate>
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[DotMFMManager defaultManager] setMfmDelegate:self];
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    if (self.exitFlag)
    {
        for (DotDevice *device in self.mfmDevices)
        {
...
- (void)updateCellDisconnect:(DotDevice *)device
{
...
- (void)startMFM
{
    ...
    /// The start MFM sdk method, parameter is DotDevice list.
    [[DotMFMManager defaultManager] startMFM:self.mfmDevices];
...
- (void)stopMFM
{
    ...
    /// The stop MFM sdk method, parameter is DotDevice list.
    [[DotMFMManager defaultManager] stopMFM:self.mfmDevices];
...
#pragma mark - DotConnectionDelegate
